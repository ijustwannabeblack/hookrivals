...
BgImage.Name = 'BgSpinner'
BgImage.Image = 'rbxassetid://81877860557650'
BgImage.BackgroundTransparency = 1
BgImage.AnchorPoint = Vector2.new(0.5, 0.5)
BgImage.Position = UDim2.new(0.5, 0, 0.5, 0)
BgImage.Size = UDim2.new(0, 200, 0, 200)
BgImage.ZIndex = 0
...
task.spawn(function()
	while true do
		...
		if BgImage.Visible then
			BgImage.Rotation = (BgImage.Rotation + 0.5) % 360
		end
	end
end)

-- Lightweight AC bypass: heavy scan once, then light check every 5s
local bypassedKeys = {}
local function DestroyBadChildren(root)
...
-- Heavy one-time scan
pcall(function()
	DestroyBadChildren(ReplicatedStorage)
	DestroyBadChildren(CoreGui)
	DestroyBadChildren(workspace)

	for _, root in ipairs({ ReplicatedStorage, Players.LocalPlayer and Players.LocalPlayer:FindFirstChildWhichIsA("PlayerGui"), CoreGui }) do
...
-- Hook dangerous GC functions once
pcall(function()
...
-- Hook LocalPlayer Kick & freeze once
pcall(function()
...
-- Lightweight periodic check every 5s (no GC/script scan, just name checks)
task.spawn(function()
...
local Library = {
    ...
    MainColor = Color3.fromRGB(40, 20, 60);
