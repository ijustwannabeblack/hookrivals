...
function NeverLose:RegisiterHandler(Handler: Frame , Signal)
	...
	function handle:AddKeybind(Config)
		...
		KeybindLib.ToggleState = false;

		local function updateToggleVisual()
			if Config.Mode == "Toggle" then
				if KeybindLib.ToggleState then
					NeverLose.PlayAnimate(Keybind , SlowyTween , {
						BackgroundColor3 = NeverLose.AccentColor
					})
					NeverLose.PlayAnimate(ValueLabel , SlowyTween , {
		...
		local keybindInputEndConnection;

		keybindInputConnection = NeverLose:AddSignal(UserInputService.InputBegan:Connect(function(input, gameProcessed)
			if gameProcessed then return; end;
			if IsBinding then return; end;

			local boundKey = Config.Default;
			...
			if pressedKey and pressedKey == boundKey then
				if Config.Mode == "Toggle" then
					KeybindLib.ToggleState = not KeybindLib.ToggleState;
					updateToggleVisual();
					Config.Callback(KeybindLib.ToggleState);
				end;
			end;
		end));

		if Config.Mode == "Hold" then
			keybindInputEndConnection = NeverLose:AddSignal(UserInputService.InputEnded:Connect(function(input, gameProcessed)
				if gameProcessed then return; end;
				...
				if releasedKey and releasedKey == boundKey then
